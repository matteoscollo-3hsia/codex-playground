...
from primer_ops.io_helpers import (
    _safe_write_json_multi,
    _safe_write_text_multi,
)
from primer_ops.openai_helpers import (
    _REQUEST_TIMEOUT_SECONDS,
    _call_openai_with_retries,
    _ensure_response_text,
...
_REMINDER_LINE_RE = re.compile(
    r"^\s*\(here\s+copy\s+and\s+paste.*\)\s*$", re.IGNORECASE
)
_REMINDER_SENTENCE = (
    "(here copy and paste introduction from 'company and industry intro' step 1)"
)
...
def resolve_output_targets(
    output_dir: str | None, lead: dict[str, Any]
) -> dict[str, Any]:
    company_name = _extract_company_name(lead)
...
def _post_process_prompt(
    prompt_original: str,
    prev_sheet_name: str | None,
    prev_sheet_output_text: str,
    ...
    if re.search(pattern, cleaned, flags=re.MULTILINE | re.DOTALL):
        prompt_final = re.sub(
            pattern, context_block, cleaned, flags=re.MULTILINE | re.DOTALL
        )
    else:
    ...
    for sheet in sheets:
        ...
        sanitized_sheet: dict[str, Any] = {
            ...
            "deep_research_requested": _coerce_bool(
                sheet.get("deep_research_requested"), False
            ),
            "deep_research_effective": _coerce_bool(
                sheet.get("deep_research_effective"), False
            ),
            "deep_research_error_reason": _coerce_str(
                sheet.get("deep_research_error_reason")
            ),
            "steps": [],
        }
        ...
        if isinstance(steps, list):
            for step in steps:
                ...
                if not response_text:
                    response_text = (
                        _extract_output_text_from_response(step.get("response")) or ""
                    )
                ...
                sanitized_step: dict[str, Any] = {
                    ...
                    "reasoning_effort_requested": _coerce_str(
                        step.get("reasoning_effort_requested")
                        or step.get("reasoning_effort")
                    ),
                    "reasoning_effort_effective": _coerce_str(
                        step.get("reasoning_effort_effective")
                    ),
                    "web_search": _coerce_bool(
                        step.get("web_search"), sanitized_sheet["web_search"]
                    ),
                    "deep_research_requested": _coerce_bool(
                    ...
                    "deep_research_effective": _coerce_bool(
                        step.get("deep_research_effective"), False
                    ),
                    "deep_research_error_reason": _coerce_str(
                        step.get("deep_research_error_reason")
                    ),
...
def _resolve_template_path() -> Path | None:
    ...
    if candidate.is_dir():
        raise SystemExit(
            f"ERROR: PRIMER_WORD_TEMPLATE_PATH is a directory: {candidate}"
        )
    try:
...
def _is_verbose() -> bool:
    return any(
        os.getenv(name, "").strip().lower() in ("1", "true", "yes", "y", "on")
...
def generate_primer(
    ...
    include_headings_effective = (
        include_headings
        if include_headings is not None
        else get_include_headings(False)
    )
    ...
    def parse_sheet_filter(filter_value: str, available: list[str]) -> set[str]:
        if "," in filter_value:
            requested = [
                item.strip() for item in filter_value.split(",") if item.strip()
            ]
    ...
    runnable_sheets = [
        name for name in workbook.sheetnames if is_runnable_sheet(workbook[name])
    ]
    ...
    if sheet is None:
        if include:
            selected_sheets = [
                ...
                if name in parse_sheet_filter(include, selected_sheets)
            ]
        if exclude:
    ...
    if isinstance(sources_payload.get("sheets"), list):
        ...
        if removed_count:
            print(
                f"Removed {removed_count} sheet(s) from sources.json "
                "not present in prompt library."
            )

    prev_sheet_output_text = get_initial_context(sources_payload)
    prev_sheet_name: str | None = (
        "seed:intro" if prev_sheet_output_text.strip() else None
    )
    ...
    for sheet_index, sheet_name in enumerate(selected_sheets, start=1):
        ...
        web_search_value = _first_right_value(
            ws, web_search_cell.row, web_search_cell.column
        )
        ...
        gpt_model_cell = _find_anchor_exact(
            ws, "GPT Model", start_row=instructions_cell.row
        )
        if gpt_model_cell is not None:
            gpt_model_value = _first_right_value(
                ws, gpt_model_cell.row, gpt_model_cell.column
            )
        ...
        deep_research_requested = _normalize(str(deep_research_value)).startswith(
            "enable"
        )
        if deep_research_requested and not web_search_enabled:
            raise SystemExit(
                "ERROR: Deep Research is Enable but Web Search is not Enable. "
                "Deep Research needs a data source."
            )
        ...
        while True:
            ...
            if step_cell is None:
                break
            step_label, step_title_clean = _parse_step_title(
                step_cell.value, step_number
            )
            if step_title_clean:
            ...
            if existing_step_entry is None:
                ...
                if "citations" in step_entry and not isinstance(
                    step_entry["citations"], list
                ):
                    step_entry["citations"] = []
            try:
                next_step_cell = _find_step_anchor(
                    ws, step_number + 1, start_row=step_cell.row + 1
                )
                end_row = (
                    next_step_cell.row if next_step_cell is not None else ws.max_row + 1
                )
                suggested_prompt_cell = _find_anchor_exact_in_window(
                    ws,
                    "Suggested Prompt",
                    start_row=step_cell.row,
                    end_row=end_row,
                )
                if suggested_prompt_cell is None:
                    raise ValueError(
                        f"ERROR: missing anchor: Suggested Prompt (step {step_number})"
                    )
                suggested_prompt_value = _first_right_value(
                    ws,
                    suggested_prompt_cell.row,
                    suggested_prompt_cell.column,
                    limit=20,
                )
                if (
                    not isinstance(suggested_prompt_value, str)
                    or not suggested_prompt_value.strip()
                ):
                    raise ValueError("ERROR: missing anchor: Suggested Prompt")

                prompt_original = _replace_placeholders(
                    suggested_prompt_value, lead_plus
                )
                ...
                call_label = (
                    f"[sheet {sheet_index}/{len(selected_sheets)}][step {step_number}]"
                )

                if (
                    resume
                    and existing_step_entry is not None
                    and _step_is_completed(existing_step_entry)
                ):
                    ...
                    web_tool_type: str | None = (
                        "web_search" if web_search_enabled else None
                    )
                    deep_research_effective = False
                    ...
                    if deep_research_requested:
                        ...
                        try:
                            ...
                            model = deep_model
                            web_tool_type = (
                                "web_search_preview" if web_search_enabled else None
                            )
                            ...
                            if (
                                ...
                                and _model_supports_reasoning_effort(model)
                            ):
                            ...
                            try:
                                ...
                                error_message = (
                                    f"{type(err_fallback).__name__}: {err_fallback}"
                                )
                    else:
                        request_kwargs = {
                            "model": model,
                    ...
                    step_entry["deep_research_error_reason"] = (
                        deep_research_error_reason
                    )
                    step_entry["error"] = error_message
                    step_entry["response_text"] = (
                        output_text.strip() if output_text else ""
                    )
                    if citations:
                    ...
                    if (
                        deep_research_error_reason
                        and sheet_entry["deep_research_error_reason"] is None
                    ):
                        sheet_entry["deep_research_error_reason"] = (
                            deep_research_error_reason
                        )
